...
parser = argparse.ArgumentParser(description='MNIST Lenet_300_100 Compression')

# Basic Setting
parser.add_argument('--seed', default=1, type = int, help = 'set seed')
parser.add_argument('--base_path', default='./result/mnist_300_100/', type = str, help = 'base path for saving result')
parser.add_argument('--model_path', default='test_run/', type = str, help = 'folder name for saving model')

# Training Setting
parser.add_argument('--nepoch', default = 200, type = int, help = 'total number of training epochs')
...
parser.add_argument('--sigma1', default = 0.02, type = float, help = 'sigma_1 in prior')
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=args.batch_train, shuffle=True,
                                               num_workers=4)
    ...
    net = my_resnet.Lenet_300_100_sparse()
    net.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 50 and epoch < 150:
            proposal_a = 200
            proposal_b = 400 * (150.0 - epoch) / 100.0 + 200 * (1 - (150.0 - epoch) / 100.0)
        if epoch >= 150:
            proposal_b = 200
    ...
    f = open(filename, 'wb')
    pickle.dump([train_loss_path, train_accuracy_path, test_loss_path,
                 test_accuracy_path, sparsity_path], f)
    f.close()
