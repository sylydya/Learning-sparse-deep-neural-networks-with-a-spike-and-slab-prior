...
class my_Net(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1 = Mask_Linear(2000, 6)
        self.fc2 = Mask_Linear(6,4)
        self.fc3 = Mask_Linear(4,3)
        self.fc4 = Mask_Linear(3,1)
...
def main():
    ...
    for iter in range(max_loop):
        ...
        for MH_iter in range(MH_loop):
            ...
            prior = 0
            for name, para in net.named_parameters():
                prior = prior + para.mul(current_mask[name]).pow(2).div(-2*prior_sigma).sum() + para.mul(1-current_mask[name]).pow(2).div(-2*prior_sigma_0).sum() \
            ...
            for name, para in net.named_parameters():
                probability = para.abs().mul(-proposal_b).exp().mul(proposal_a).add(1).pow(-1)
                new_mask[name] = torch.where(torch.rand_like(para) < probability, torch.ones_like(para), torch.zeros_like(para))
            ...
            new_prior = 0
            for name, para in net.named_parameters():
                new_prior = new_prior + para.mul(new_mask[name]).pow(2).div(-2*prior_sigma).sum() + para.mul(1-new_mask[name]).pow(2).div(-2*prior_sigma_0).sum() \
            ...
            with torch.no_grad():
                log_MH_ratio = ((new__target - current_target)/temperature + (new_ones - current_ones)/temperature * np.log(lambda_n/(1-lambda_n)) + (current_log_proposal - new_log_proposal)).item()
